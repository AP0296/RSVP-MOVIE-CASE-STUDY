...
SELECT COUNT(movie_id) as total_director_mapping
	FROM director_mapping;
-- Total no. of rows 3867

SELECT COUNT(movie_id) as total_genre
	FROM genre;
...
SELECT COUNT(id) AS total_names
	FROM names;
-- -- Total no. of rows 25735


SELECT COUNT(movie_id) AS total_names
	FROM names;
-- -- Total no. of rows 25735

			/*OR*/
            
-- Using Information_schema we can derive all the rows at once
...
/*-- For Text Format : Date we cannot use OR condition. 
				     Else wise for General Format we  can use OR condition*/
...
|	3			|	 824			|
+---------------+-------------------+ */
-- March month is having more number of movies released
...
SELECT 
		COUNT(distinct id) AS movie_number
	FROM movie
		WHERE country REGEXP 'India|USA' AND YEAR='2019';
        
-- O/P - 1059 movies were produced in the USA or India in the year 2019
...
-- O/P - There are 13 unique genres are present in the data set
...
SELECT genre, COUNT(distinct movie_id ) as Movie_produced
	FROM genre 
		GROUP BY genre 
			ORDER BY Movie_produced DESC;
-- O/P Drama has the highest number of movies produced overall = 4285 
...
SELECT COUNT(movie_id) AS MOVIE_n
	FROM genre;
 
SELECT SUM(genre_count) AS genre_count_1
	FROM 
(
SELECT
  movie_id,
  COUNT(genre) AS genre_count
	FROM genre
		GROUP BY movie_id
			HAVING genre_count= 1 ) AS gen_count;
-- O/P - 3289 movies belong to only one genre
...
-- Thriller genre is among top 3 in the genre_rank
...
-- O/P the median rating 7 has highest in number with value 2257
...
-- Type your code below:
SELECT COUNT(movie_id) AS movie_cnt, median_rating
	FROM ratings r 
		INNER JOIN movie m
			ON m.id = r.movie_id
...
---/*LANGUAGE WISE APPROACH*/
SELECT languages, SUM(total_votes) AS Tv
	FROM ratings r 
		INNER JOIN movie m
			ON m.id = r.movie_id 
